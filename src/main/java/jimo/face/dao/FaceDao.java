...
public class FaceDao {
    ...
    /***
     * 通过faceToken删除FaceSet中的人脸信息
     * @param faceTokens
     */
    public void  deleteFaceToFaceSet(String faceTokens){
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
        MultiValueMap<String, String> map = new LinkedMultiValueMap<>();
        map.add("api_key", apiKey);
    ...
    /***
     * 通过filePath查找FaceSet中的匹配度最高的人脸信息FaceToken
     * @param filePath
     */
    public FaceEntity  searchFaceToFaceSet(String filePath){
        HttpHeaders headers = new HttpHeaders();
        ...
        MultiValueMap<String, HttpEntity<?>> multipartBody =
                multipartBodyBuilder.build();
        ResponseEntity<SearchFace> searchFaceResponseEntity =
                restTemplate.postForEntity("https://api-cn.faceplusplus.com/facepp/v3/search",
                        multipartBody,SearchFace.class);
        System.out.println(searchFaceResponseEntity);
        SearchFace e = searchFaceResponseEntity.getBody();
//        System.out.println(e);
        assert e != null;
        CompareResponseEntity compareResponseEntity = e.results.get(0);//默认为第一个最高匹配度数据信息
        if (compareResponseEntity.getConfidence() != null && compareResponseEntity.getConfidence() >= e.getThresholds().e5) {
            FaceEntity faceEntity = new FaceEntity();
            faceEntity.setFace_token(compareResponseEntity.getFace_token());
            return faceEntity;
        } else {
            return null;
        }
     ...
     */
    @Data
    public static class SearchFace {
        private ThresholdsResponseEntity thresholds;
        private List<CompareResponseEntity> results;
    }
}